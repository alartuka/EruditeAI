...
export default function Generate() {
  ...
  const handleSubmit = async () => {
    if (!isSignedIn) {
      toast.error("Please sign in first to generate flashcards.");
      return;
    }
    ...
    try {
      ...
      if (userDocSnap.exists()) {
        ...
        if (userData.find((f) => f.name === name)) {
          toast.error('A flashcard set with this name already exists.');
          return;
        } else {
          userData.push({ name });
      ...
      toast.error('An error occurred while saving flashcards. Please try again.');
    }
  }
  ...
  return (
    <Container maxWidth="md">
      ...
      <Toaster /> {/* Ensure the Toaster is rendered */}
      <Box sx={{ my: 4 }}>
        ...
        <Box display="flex" justifyContent="center">
          <Button
            ...
            disabled={generating}
            sx={{ mt: 2, backgroundColor: '#004d40', '&:hover': { backgroundColor: '#00332e' }, width: 'auto' }}
          >
